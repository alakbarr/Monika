...
from .invoker import invoke_text, invoke_json, invoke_agent, invoke_chat, invoke_custom
...
async def run_one_model_for_case(run_id, task_id, category, mode, model_name, case, settings,
    ...
    async with semaphore:
        async with get_session() as session:
            ...
            if inv.error is None and output_for_judge:
                try:
            ...
            det_pass = deterministic["pass_rate"] if deterministic else None
            overall = None
            if judge_overall is not None:
                overall = judge_overall if det_pass is None else 0.6 * judge_overall + 0.4 * (det_pass * 10)

            try:
                cost = cost_usd(model_name, inv.input_tokens, inv.output_tokens)
            ...
            row = BenchmarkResult(
                ...
                input_tokens=inv.input_tokens, output_tokens=inv.output_tokens, cost_usd=round(cost, 6),
